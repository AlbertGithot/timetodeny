...
def _find_llama_server_binary() -> Path | None:
    ...
    candidates.extend(
        [
            project_root / "llamaserver" / "llama.cpp" / "build" / "bin" / "Release" / "llama-server.exe",
            project_root / "llamaserver" / "llama.cpp" / "build" / "bin" / "llama-server.exe",
            project_root / "llamaserver" / "llama.cpp" / "build" / "Release" / "llama-server.exe",
            project_root / "llamaserver" / "llama.cpp" / "build" / "llama-server.exe",
            project_root / "llamaserver" / "llama.cpp" / "llama-server.exe",
            project_root / "llamaserver" / "llama-server.exe",
...
def ensure_llama_server(model: ModelRegistry | None = None) -> None:
    ...
    binary = _find_llama_server_binary()
    if not binary:
        raise RuntimeError("llama-server binary was not found. Run .\\windows.ps1 repair to build llama.cpp.")
